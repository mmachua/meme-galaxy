...
urlpatterns = [
    ...
    path(r'^reset-password/$',auth_views.PasswordResetView.as_view(),{'template_name':
    ...
    path(r'^reset-password/done/$', auth_views.PasswordResetDoneView.as_view(template_name='login/password_reset_done.html'),
     name='password_reset_done'),
    #path(r'^reset-password/confirm/$',PasswordResetConfirmView.as_view() , name='password_reset_confirm'),
    path(r'^reset-password/confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
    auth_views.PasswordResetConfirmView.as_view(),{'template_name':'login/reset_password_confirm.html'},
    name='password_reset_confirm'),
    
    path(r'^reset-password/done/$', auth_views.PasswordResetCompleteView.as_view, 
